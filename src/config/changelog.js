...
const CHANGELOG_DATA = [
    {
        ...
        items: [
            ...
            ' 新增磁力播放浮动进度条：右下角toast设计，两行布局，清晰阶段文字反馈',
            ' 新增磁力播放本地优先快速路径：复用已有torrent，跳过重复解析，不重复连接或下载',
            ' 新增6阶段进度反馈：连接→解析→选择→下载→就绪→播放，每个阶段有清晰状态文字',
            ' 新增进度条二级阶段文本：支持warning/success/error颜色变体',
            ' 重构外链页架构：PlayUrlController状态机，6状态单点切换',
            ' 新增智能输入识别：inputRecognizer纯函数，支持magnet/URL/本地路径/Hash自动识别',
            ' 新增文件列表渲染器和磁力解析适配器',
            ' 新增URL历史管理器和外链页专用样式',
            ' 新增Node.js运行环境自动检测和便携版按需下载',
            ' 修复webtorrent在Node.js<22下ESM加载失败',
            ' 历史功能整合：移除独立历史抽屉，统一到历史页面管理',
            ' 浮动条自动消失：播放器打开后自动隐藏，不依赖IPC事件链',
            ' 修复关闭播放器后再次播放hang：改用同步查找替代c.get()异步查找',
            ' 修复IPC订阅白名单拦截：修正safeOn→on API名',
            ' 修复浮动条状态不更新和播放后不消失',
            ' 修复进度订阅覆盖"准备就绪"状态',
            ' 极简化浮动条逻辑：移除IPC事件依赖，改为await主流程驱动'
        ]
    },
    {
        version: 'v1.6.0',
        ...
        items: [
            ...
            ' 删除ipcHandler.js中重复的torrent缓存获取逻辑（94行死代码）'
        ]
    },
    {
        version: 'v1.5.2',
        date: '2026-03-02',
        items: [
            ' 修复发布版m3u8视频无法播放：hls.js改为本地文件加载，解决打包后模块加载问题',
            ' 修复依赖打包问题：配置pnpm使用hoisted模式，解决asar中依赖缺失问题',
            ' 修复安装程序检测旧版本问题：改进NSIS脚本，支持自动检测原安装目录',
            ' 改进错误提示：提供更准确的错误信息，帮助用户理解问题原因',
            ' 新增复制错误信息功能：错误界面添加复制按钮，方便用户反馈问题',
            ' 移除播放器悬停变暗效果：提升观看体验'
        ]
    },
    {
